...
from .modules import encoder_cbhg, post_cbhg, prenet, reference_encoder, GuidedAttention, Postnet
from .rnn_wrappers import DecoderPrenetWrapper, ConcatOutputAndAttentionWrapper, ZoneoutWrapper, FrameProjection
...
class Tacotron():
    ...
    def initialize(self, inputs, input_lengths, mel_targets=None, stop_token_targets=None, is_training=False,
        ...
        with tf.variable_scope('inference') as scope:
            ...
            decoder_output = tf.reshape(decoder_outputs, [batch_size, -1, hp.num_mels])  # [N, T_out, M]
            # stop_token_outputs = tf.reshape(stop_token_outputs, [batch_size, -1],
            #                                 name="stop_token_prediction")  # [N, T_out, M]
            ...
            # # Add post-processing CBHG:
            # post_outputs = post_cbhg(decoder_output, hp.num_mels, is_training)  # [N, T_out, 256]
            # print(post_outputs)
            # mel_outputs = tf.reshape(post_outputs, [batch_size, -1, hp.num_mels])
            # mel_outputs = tf.layers.dense(post_outputs, hp.num_freq)  # [N, T_out, F]
            #
            # # Postnet
            ...
            #
            with tf.variable_scope('mel_outputs'):
            ...
            log('  mel_outputs:              %d' % mel_outputs.shape[-1])
    ...
    def add_loss(self):
        ...
        with tf.variable_scope('loss') as scope:
            ...
            # TODO test right
            """
            entropy = tf.contrib.distributions.Categorical(probs=self.alignments).entropy()
            entropy_loss = (entropy / tf.log(self.alignments.get_shape()[1])).mean()
