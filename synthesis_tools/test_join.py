...
class AudioJoiner:
    ...
    # 加载声音文件
    def load_songfile(self, songPath, frame_rate=8000, channels=1):
        # song = None
    ...
    def combine_two_song_file(self, songPath1, songPath2, frame_rate=8000, channels=1, adjectDb=True):
    ...
    def combine_two_song(self, song1, song2, adjectDb=True):
        if not song1 or not song2:
            return None
        if adjectDb:
            # 取得两个MP3文件的声音分贝
    ...
    def export_audio_file(self, targetPath="output.wav", audio_format="wav",
                          bitrate="256k", frame_rate=24000, channels=1):
...
def test_demo():
    ...
    # 加入单个文件
    # audio_joiner.join_singlefile(r"/home/btows/SDB/1_1.wav")
    # audio_joiner.join_singlefile(r"/home/btows/SDB/1_3.wav")
    # 通过列表添加
    filelist = list()
    filelist.append("/media/btows/SDB/working/project/join_tts/gfyh8k/1_1.wav")
    filelist.append("/media/btows/SDB/working/project/join_tts/out_tts/tts1.wav")
    audio_joiner.join_list(filelist)

    # audio_joiner.join_singlefile(r"d:\byby7.wav")
    outfile = "/media/btows/SDB/working/project/join_tts/1_1.wav"
