...
from datasets import blizzard, ljspeech, blizzard2013, chinese_dataset
...
def preprocess_chinese(args):
    in_dir = os.path.join(args.base_dir, 'database/chinese_demo')
    mode = args.mode
    out_dir = os.path.join(args.base_dir, args.output)
    os.makedirs(out_dir, exist_ok=True)
    metadata = chinese_dataset.build_from_path(in_dir, out_dir, args.num_workers, mode=mode, tqdm=tqdm)
    write_metadata(metadata, out_dir)
...
def write_metadata(metadata, out_dir):
    ...
    # print('Max input length:  %d' % max(len(m[3]) for m in metadata))
    # print('Max output length: %d' % max(m[2] for m in metadata))
    print("Max features length: %d" % max(m[3] for m in metadata))
...
def main():
    ...
    parser.add_argument('--dataset', default='chinese', choices=['blizzard', 'ljspeech', 'blizzard2013', 'chinese'])
